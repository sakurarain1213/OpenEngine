...
typedef MinkowskiDiff tShape;    //Boost依赖


/*
先把dir从世界坐标转换成局部坐标，然后求解局部支撑点，再转回世界坐标
*/
inline Vector3f OpenEngine::MinkowskiDiff::Support1(Vector3f dir)
{
...
inline Vector3f OpenEngine::MinkowskiDiff::Support2(Vector3f dir)
{
	TransformComponent* transform = box2->GetOwner()->GetComponent<TransformComponent>();
	Vector3f localDir = transform->GlobalVectorToLocalVector(dir);
...
Vector3f OpenEngine::MinkowskiDiff::Support(Vector3f dir)
{
	return (Support1(dir) - Support2(dir * -1));
}

Vector3f OpenEngine::MinkowskiDiff::Support(Vector3f dir, int idx)
{
	if (idx == 1)
	{
		return (Support1(dir));
...
struct GJK
{
	...
	eStatus::_ Evaluate(const tShape& shapearg, const Vector3f& guess)
	{
		...
		m_ray = guess;
		float sqrl = length2(m_ray);
		Vector3f tempif(0, 0, 0);
		if (sqrl > 0) tempif = m_ray * -1;
		else tempif << 1.0, 0.0, 0.0;

		appendvertice(m_simplices[0], tempif);
		m_simplices[0].p[0] = 1;
		...
		do
		{
			...
			Vector3f tempif(m_ray * -1);
			appendvertice(cs, tempif);
			...
			for (U i = 0; i < 4; ++i)
			{
				Vector3f diff;
				diff=w-lastw[i];
				if (length2(diff) < GJK_DUPLICATED_EPS)
				{
					found = true;
					break;
				}
	...
	bool EncloseOrigin()
	{
		...
		case 2:
		{
			...
			for (U i = 0; i < 3; ++i)
			{
				...
				if (length2(p) > 0)
				{
					...
					appendvertice(*m_simplex, tempif);
					if (EncloseOrigin()) return (true);
					removevertice(*m_simplex);
				}
			}
		...
		case 3:
		{
			Vector3f n = m_simplex->c[1]->w - m_simplex->c[0]->w   . cross(
	...
	{
		return (a[1] * b[2] * c[0] + a[2] * b[0] * c[1] -
		...
		float* w, U& m)
	{
		...
		if (l > GJK_SIMPLEX2_EPS)
		{
			...
			else
			{
				...
				Vector3f tempif(a + d * t);

				return (length2(tempif));
			}
		}
		return (-1);
	}
		...
		float* w, U& m)
	{
		...
		Vector3f n = dl[0].cross( dl[1]);
		const float l = length2(n);
		if (l > GJK_SIMPLEX3_EPS)
		{
			...
			if (mindist < 0)
			{
				...
				Vector3f tempif(dl[1].cross(b - p));

				w[0] = length(tempif) / s;
				tempif=(dl[2].cross(c - p));
				w[1] = length(tempif) / s;
				w[2] = 1 - (w[0] + w[1]);
			}
			return (mindist);
		...
		float* w, U& m)
	{
		...
		if (ng && (abs(vl) > GJK_SIMPLEX4_EPS))
		{
			...
			{
				const U j = imd3[i];
				const float s = vl * (d.dot (dl[i].cross( dl[j])));
				if (s > 0)
				{
...
struct EPA
{
	...
	eStatus::_ Evaluate(GJK& gjk, Vector3f& guess)
	{
		...
		if ((simplex.rank > 1) && gjk.EncloseOrigin())
		{
			...
			if (m_hull.count == 4)
			{
				...
				m_result.p[0] = 
					length((outer.c[1]->w - projection.cross(
						outer.c[2]->w - projection)));
				m_result.p[1] = 
					length((outer.c[2]->w - projection.cross(
						outer.c[0]->w - projection)));
				m_result.p[2] =
					length((outer.c[0]->w - projection.cross(
		...
		m_normal = guess * -1;
		const float nl = length(m_normal);
		if (nl > 0)
	...
	{
		Vector3f ba = b->w - a->w;
		Vector3f n_ab = ba.cross(face->n);   // Outward facing edge normal direction, on triangle plane
		float a_dot_nab = a->w.dot(n_ab);  // Only care about the sign to determine inside/outside, so not normalization required

		if (a_dot_nab < 0)
		{
			// Outside of edge a->b

			const float ba_l2 = length2(ba);
			const float a_dot_ba = a->w.dot( ba);
			const float b_dot_ba = b->w.dot(ba);
			...
			else
			{
				...
				const float a_dot_b = a->w.dot(b->w);

				float t = (length2(a->w) *length2(b->w) - a_dot_b * a_dot_b) / ba_l2;
...
*/
void OpenEngine::NarrowPhaseGJKEPA::UpdateManifolds(std::vector<std::shared_ptr<ContactManifold>>& manifolds)
{
...
void OpenEngine::NarrowPhaseGJKEPA::CollideDetection(std::vector<RigidBodyPair>& rigidBodiesPairs, std::vector<std::shared_ptr<ContactManifold>>& manifolds)
{
	for (auto pair : rigidBodiesPairs)
	{
		...
		if (Penetration(pair, guess, result))
		{
			...
			manifold->colliderA = pair.first;
			manifold->colliderB = pair.second;
...
bool OpenEngine::NarrowPhaseGJKEPA::Penetration(RigidBodyPair& pair, Vector3f& guess, sResults& result)
{
	...
	case GJK::eStatus::Inside:
	{
		...
		Vector3f tempif(guess * -1);

		EPA::eStatus::_ epa_status = epa.Evaluate(gjk, tempif);
...
void OpenEngine::NarrowPhaseGJKEPA::GenerateTangents(ContactPoint& contactPoint)
{
	if (contactPoint.normal[0] >= 0.57735f)
	{
		contactPoint.tangent1[0] = contactPoint.normal[1];
		contactPoint.tangent1[1] = -contactPoint.normal[0];
		contactPoint.tangent1[2] = 0;
	}
	else
	{
		contactPoint.tangent1[0] = 0;
		contactPoint.tangent1[1] = contactPoint.normal[2];
		contactPoint.tangent1[2] = -contactPoint.normal[1];
	}
	Normalize(contactPoint.tangent1);
	contactPoint.tangent2 =contactPoint.normal.cross(contactPoint.tangent1);
